...
- [ ] 2.1 `parse_time_sig_rejects_missing_slash` — asserts
  `parse_time_sig("44")` returns `Err` whose message contains "expected N/N".
- [ ] 2.2 `parse_time_sig_rejects_non_numeric_numerator` — asserts
  `parse_time_sig("x/4")` returns `Err` whose message contains "numerator".
- [ ] 2.3 `parse_time_sig_rejects_non_numeric_denominator` — asserts
  `parse_time_sig("4/x")` returns `Err` whose message contains "denominator".

## 3. Confirm no regressions
- [ ] 3.1 Confirm the existing happy-path rhythm tests (`test_time_sig_parse`,
