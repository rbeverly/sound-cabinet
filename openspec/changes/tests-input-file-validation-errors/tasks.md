...
- [ ] 1.1 `pattern_file_requires_rhythm_contour_or_motif` — asserts
...
- [ ] 1.2 `pattern_file_rejects_direct_emphasis_length_mismatch` — asserts
...
- [ ] 1.3 `pattern_file_rejects_contour_rest_without_rhythm_rest` — asserts
  `PatternFile::from_yaml` with `rhythm.hits: ["1/4", "1/4"]` and
  `contour: [root, "~"]` returns `Err` whose message contains
  "contour position 2 is '~'".
- [ ] 1.4 `pattern_file_rejects_motif_rhythm_contour_mismatch` — asserts
...
- [ ] 1.5 `pattern_file_rejects_motif_emphasis_length_mismatch` — asserts
...
- [ ] 2.1 `song_file_rejects_empty_parts` — asserts `SongFile::from_yaml` with
  `parts: {}` and `arrangement: [verse]` returns `Err` whose message contains
  "must have at least one part".
- [ ] 2.2 `song_file_rejects_empty_arrangement` — asserts `SongFile::from_yaml`
  with one valid part and `arrangement: []` returns `Err` whose message
  contains "arrangement must have at least one entry".
- [ ] 2.3 `song_file_rejects_part_motif_rhythm_contour_mismatch` — asserts
...
- [ ] 3.1 `drum_pattern_rejects_empty_voices` — asserts `DrumPattern::from_yaml`
  with `voices: []` returns `Err` whose message contains
  "must have at least one voice".
- [ ] 3.2 `drum_pattern_rejects_emphasis_length_mismatch` — asserts
...
- [ ] 4.1 Confirm the existing pattern tests (`test_mismatched_lengths_rejected`,
