...
- [ ] 1.1 Create `install.sh` at the repo root. Set `#!/usr/bin/env bash` shebang. First non-comment line: `set -euo pipefail`. Define `IFS=$'\n\t'`. Add `trap 'echo "ERROR at line $LINENO (step: ${CURRENT_STEP:-unknown})" >&2' ERR`. Use a `CURRENT_STEP=...` assignment at the top of each major section so the trap is informative.
- [ ] 1.2 Define top-level constants near the top of the file:
  - `REPO_OWNER="rbeverly"`
...
- [ ] 1.5 Implement `fetch_releases_json(api_base_url)`:
...
- [ ] 1.6 Implement `parse_tags_from_json(json, include_prereleases)`:
...
- [ ] 1.7 Implement `select_version(parsed_tags_list, --version, --non-interactive)`:
...
- [ ] 1.8 Implement `determine_install_path(--prefix, --user)`:
  - `--prefix` wins.
  - `--user` → `${HOME}/.local/bin`.
...
- [ ] 1.9 Implement `linux_alsa_check()`:
...
- [ ] 1.10 Implement `download_and_verify(releases_base_url, tag, triple, tmpdir)`:
...
- [ ] 1.11 Implement `install_binary(tmpdir, tag, triple, install_path)`:
...
- [ ] 1.12 Implement `path_check_and_hint(install_path)`:
...
- [ ] 1.13 Final summary block. After the install completes:
...
- [ ] 1.14 Implement `--dry-run` gating. Every destructive call site (curl downloads, install, sudo, file moves) checks a `DRY_RUN` flag and, when set, prints `[dry-run] <command>` instead of executing. The version-resolution API call IS still made in dry-run (it's read-only) so the rest of the pipeline can be exercised end-to-end.
- [ ] 1.15 Length budget: aim for ≤ 200 lines including blank lines and comments, ≤ 250 hard ceiling. If the implementing agent exceeds the ceiling, that's a signal to split logic into helper functions and trim — not to add a Rust subcommand.

## 2. Test harness

- [ ] 2.1 Create `tests/install/` directory.
- [ ] 2.2 Create `tests/install/fixtures/curl` — bash script, `chmod +x`. Behavior:
...
- [ ] 2.3 Create `tests/install/fixtures/uname` — bash script. If `$1 == "-s"` echo `${MOCK_UNAME_S:-Linux}`. If `$1 == "-m"` echo `${MOCK_UNAME_M:-x86_64}`. With no arg, echo `${MOCK_UNAME_S} ${MOCK_UNAME_M}`.
- [ ] 2.4 Create `tests/install/fixtures/sudo` — bash script. `echo "sudo: $*" >> "${SUDO_LOG:-$TMPDIR/sudo.log}"`. Then `exec "$@"` so the wrapped command runs (but in `--dry-run` mode the wrapped command itself will be a no-op).
- [ ] 2.5 Create `tests/install/fixtures/releases.json` — hand-crafted JSON mimicking the GitHub Releases API shape. Include at least 7 entries:
...
- [ ] 2.6 Create `tests/install/fixtures/binaries/` directory containing:
...
- [ ] 2.7 Create `tests/install/lib.sh` — shared test helpers:
...
- [ ] 2.8 Create `tests/install/run.sh` — runner. `cd "$REPO_ROOT"`. `for t in tests/install/*.test.sh; do bash "$t"; done`. Print `PASS: <t>` / `FAIL: <t>` per file. Track totals; exit 1 if any fail.
- [ ] 2.9 Create each `*.test.sh` file per the proposal's test scenarios list. Each file sources `tests/install/lib.sh`, calls `setup_test`, runs `run_install` with the scenario's args + env vars, calls a series of asserts, calls `teardown_test`. Keep each file ≤ 30 lines for readability.

## 3. README update

- [ ] 3.1 In `README.md`'s `## Install` section, REPLACE the existing **"Pre-built binaries"** subsection with a new **"Quick install"** subsection at the top of `## Install`. Content:
...
- [ ] 3.2 RETAIN the **"Build from source"** subsection unchanged. RETAIN platform dependencies (ALSA on Linux) but link it from the Quick Install section so users hitting the ALSA warning can find the fix easily.
- [ ] 3.3 The existing `sudo mv sound-cabinet /usr/local/bin/` manual install block: trim it down to a single short paragraph titled "Manual install" pointing readers at the GitHub Releases page and the asset naming convention. Most users should use the one-liner; this section is for the air-gapped / no-curl case.

## 4. Spec delta

- [ ] 4.1 Author the ADDED requirements in `specs/install-script/spec.md` per the spec authored alongside this proposal. Validate with `openspec validate install-script-and-wizard --strict`.

## 5. Verification

- [ ] 5.1 `openspec validate install-script-and-wizard --strict` passes.
- [ ] 5.2 `bash -n install.sh` passes (syntax check).
- [ ] 5.3 `bash tests/install/run.sh` passes — every `*.test.sh` exits 0, the runner reports zero failures.
- [ ] 5.4 The implementing agent SHOULD attempt `shellcheck install.sh tests/install/*.sh` if shellcheck is on PATH. If not, print one `WARN: shellcheck not available in sandbox — skipping lint` line and proceed. DO NOT install shellcheck as part of the change; DO NOT fail the implementation if it's absent.
- [ ] 5.5 Structural sanity checks:
