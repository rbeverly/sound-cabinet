...
- [ ] 1.1 In `src/engine/engine.rs`, define a private module-level
...
- [ ] 1.3 In `try_handle_arp`
  ...
  ```
- [ ] 1.4 Just before the `Vec::with_capacity(total_steps)` call at
  ...
  ```
- [ ] 1.5 Confirm `octaves` is already bounded (see
...
- [ ] 2.2 Add a unit test `arp_rejects_non_positive_rate` covering
