...
- [x] 1.1 In `src/generate/pattern.rs`, inside `SongFile::validate` (the loop
...
- [x] 2.1 In `src/generate/motif.rs`, harden against a short emphasis array
...
## 3. Tests
- [x] 3.1 Add a unit test `test_song_part_short_emphasis_errors_not_panics` in
...
- [x] 3.2 Add a unit test in `src/generate/pattern.rs` asserting
  `SongFile::from_yaml` returns `Err` for a part whose motif emphasis length
  differs from its rhythm length.
- [x] 3.3 Confirm existing song tests (`test_generate_song`,
