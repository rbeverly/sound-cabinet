...
The script SHALL detect the host OS and architecture via `uname -s` and `uname -m`, and SHALL map them to exactly one of three supported Rust target triples:
...
Any other combination — including `Darwin` + `x86_64` (Intel Mac), `Linux` + `armv7l` / `i686`, `*BSD`, `MINGW*`, etc. — SHALL produce a clear "no pre-built binary for your platform" error and SHALL exit non-zero before any network call.
...
#### Scenario: Intel Mac is not supported
- **GIVEN** `uname -s` returns `Darwin` and `uname -m` returns `x86_64`
- **WHEN** the script runs
- **THEN** the script prints an error naming Intel Mac as unsupported
- **AND** the message points the user at the "Build from source" instructions in the README
...
For the resolved (`<tag>`, `<triple>`) pair, the script SHALL download two files into a `mktemp -d` workspace:

- The binary: `<releases-base-url>/<tag>/sound-cabinet-<tag>-<triple>`
- The checksum: `<releases-base-url>/<tag>/sound-cabinet-<tag>-<triple>.sha256`

Both downloads SHALL use `curl -fSL --proto =https --tlsv1.2` so HTTPS is enforced and a non-2xx response aborts the script. After download, the script SHALL run `sha256sum -c` (Linux) or `shasum -a 256 -c` (macOS) against the downloaded `.sha256` file. On verification failure, the script SHALL print both the computed digest and the expected digest, SHALL leave the workspace directory intact (NOT delete it), and SHALL exit non-zero with a message naming the workspace path so an operator can inspect the corrupted files.

On verification success, the script SHALL proceed to install. On any failure, the script SHALL NOT proceed.

#### Scenario: Successful download and verification
- **GIVEN** the GitHub Releases CDN serves the binary and the matching checksum
...
#### Scenario: 404 on binary URL aborts
- **WHEN** the binary URL returns a 404 (or any non-2xx)
...
- **WHEN** the binary URL succeeds but the `.sha256` URL returns a 404
...
- **GIVEN** the downloaded binary's SHA-256 does not match the contents of the downloaded `.sha256` file
...
- `tests/install/fixtures/curl` — a bash shim that intercepts curl calls, dispatches on URL pattern, and writes fixture content to the requested `-o` path.
...
- `tests/install/fixtures/binaries/` — placeholder binary files and matching `.sha256` files for each (tag, triple) combination tested, plus at least one deliberately-corrupted `.sha256` for the mismatch test.

Every documented behavior in this spec SHALL have at least one corresponding `*.test.sh` file. `bash tests/install/run.sh` SHALL exit 0 with all tests passing.
...
- **AND** no test mocks `install.sh`'s internal functions directly — interception happens at the tool boundary (`curl`, `uname`, `sudo`)
