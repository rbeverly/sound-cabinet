...
- [x] 1.1 In `src/engine/engine.rs`, `parse_arp_options`, in the
...
- [x] 2.1 In `src/engine/engine.rs`, `try_handle_arp`, change the
...
## 3. Test
- [x] 3.1 Add a unit test (in `src/engine/engine.rs` tests) that calls
...
- [x] 3.2 Add a test that scheduling an arp with `accent 0` via
  `Engine::handle_command` on a `Command::PlayAt` whose expression is
  `... >> arp(C4, 4, accent, 0)` returns `Err` and does not panic.
- [x] 3.3 Add a regression test that `accent 2` still parses to
  `accent_every == Some(2)`.
