...
- [x] 1.1 Create `.github/workflows/release.yml`. Trigger: `on: push: tags: ['v*']`. Single workflow file with three jobs (`test`, `build`, `publish`) connected by `needs:` dependencies.
- [x] 1.2 Top-level `permissions: contents: read` so the default is least-privilege. The `publish` job declares its own `permissions: contents: write` since `softprops/action-gh-release` needs it.
- [x] 1.3 `test` job: `runs-on: ubuntu-22.04`. Steps:
...
- [x] 1.4 `build` matrix job: `needs: test`, `strategy.fail-fast: false`, `strategy.matrix.include:` three entries. Each entry sets `target` and the `os` runner. Targets:
...
- [x] 1.5 `build` matrix steps (in order):
  1. `actions/checkout@v4`
...
- [x] 1.6 `publish` job: `needs: build`, `runs-on: ubuntu-22.04`, `permissions: contents: write`. Steps:
...
- [x] 2.1 Create `RELEASING.md` at the repo root. Target length ≤ 50 lines. Sections:
...
- [x] 3.1 In README, under the "## Install" section, replace the existing **"Pre-built binaries"** subsection's body. Keep the `curl -L … | tar xz` instructions for each platform but update the URLs to match the new asset naming. Pattern:
...
- [x] 5.4 NOT in scope for this change's verification: actually pushing a smoke-test tag to verify the workflow runs end-to-end. That is the maintainer's manual acceptance step, documented in `RELEASING.md`. The implementing agent SHOULD NOT push tags.
