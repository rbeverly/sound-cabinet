...
- [x] 1.1 In `src/dsl/parser.rs`, `parse_repeat_block`, in the
...
- [x] 2.1 In `src/dsl/expand.rs`, `weighted_pick`, before calling
...
## 3. Test
- [x] 3.1 Add a unit test in `src/dsl/parser.rs` asserting
  `parse_script("repeat 1 {\n  pick [a:0]\n}\n")` returns `Err`.
- [x] 3.2 Add a test asserting `parse_script` of a `pick [a:-1]` block
  returns `Err`.
- [x] 3.3 Add a unit test in `src/dsl/expand.rs` calling
