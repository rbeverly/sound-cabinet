...
- [x] 1.1 In `src/generate/song.rs`, `run_generate_song`, immediately
...
- [x] 2.1 In `src/generate/resolver.rs`, change `active_chord` to
...
- [x] 2.2 Update the call site in `resolve_pattern`
  (`let chord = active_chord(...)`) to use `?`.

## 3. Test
- [x] 3.1 Add a unit test in `src/generate/song.rs` that builds a
  ...
  does not panic.
- [x] 3.2 Add a unit test in `src/generate/resolver.rs` asserting
