...
- [ ] 1.1 In `src/export/mod.rs::run_export`, after notes have been extracted and
...
- [ ] 2.1 In `src/export/lilypond.rs::make_rests` (line 287), at the top of the
...
## 3. Tests
- [ ] 3.1 Add a unit test `make_rests_terminates_on_infinite_gap` in
  `src/export/lilypond.rs` asserting `make_rests(f64::INFINITY, 0.0, 4.0)`
  returns without hanging (e.g. returns an empty vector).
- [ ] 3.2 Add a unit test `export_rejects_nonfinite_duration` in
...
- [ ] 3.3 Add a unit test confirming a normal finite score still exports
