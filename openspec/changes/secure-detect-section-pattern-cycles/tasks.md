...
- [ ] 1.1 In `src/dsl/expand.rs`, give `ExpansionContext` (or thread
...
- [ ] 1.2 In `ExpansionContext::expand_name`
...
- [ ] 1.3 In `ExpansionContext::duration_of` (`src/dsl/expand.rs:127`)
...
- [ ] 1.4 In `expand_pattern_ref` (`src/dsl/expand.rs:229`) and
  `duration_of_ref` (`src/dsl/expand.rs:251`), propagate errors from
  the cycle check; do not swallow them with `unwrap_or`.

## 2. Tests

- [ ] 2.1 Add a unit test `expand_rejects_direct_section_self_cycle`
...
- [ ] 3.1 Add an `## ADDED Requirements` block to
