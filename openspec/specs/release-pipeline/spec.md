...
Windows targets are explicitly out of scope for this requirement.

#### Scenario: Stable tag publishes a full release with three binaries and three checksums
- **WHEN** a tag matching `^v[0-9]+\.[0-9]+\.[0-9]+$` (e.g. `v0.1.0`, `v1.2.3`) is pushed
- **THEN** the test gate runs first and succeeds
- **AND** three matrix build jobs run in parallel after the test job, one per target triple
- **AND** each build job produces a stripped release binary named `sound-cabinet-<tag>-<triple>` and a checksum file `sound-cabinet-<tag>-<triple>.sha256`
- **AND** the publish job downloads all six artifacts and creates a GitHub Release for the tag containing all six files
...
- **WHEN** all three build matrix legs run AND any one of them fails
- **THEN** the publish job does NOT run (the `needs: build` gate fails)
- **AND** no GitHub Release is created for the tag

### Requirement: Asset naming convention is pinned

The workflow SHALL name release assets according to a fixed convention so consumers (the install script, manual downloaders, automated mirrors) can construct download URLs deterministically given a tag and target triple.

The binary asset name SHALL be exactly `sound-cabinet-<full-version-tag>-<rust-target-triple>` with no file extension, where:

- `<full-version-tag>` is the literal tag name including its leading `v` and any dash-suffix (e.g. `v0.1.0`, `v0.2.0-rc1`)
- `<rust-target-triple>` is one of `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`, `aarch64-apple-darwin`

The checksum file SHALL be named exactly `<binary-name>.sha256`. Its content SHALL be the SHA-256 digest in lowercase hex followed by two ASCII spaces followed by the binary's filename followed by a newline — the format consumed by both `sha256sum -c` (Linux) and `shasum -a 256 -c` (macOS) without modification.

#### Scenario: Asset names match the convention
- **WHEN** a tag `v0.1.0` is pushed and the workflow completes
- **THEN** the GitHub Release contains exactly these six files:
  - `sound-cabinet-v0.1.0-x86_64-unknown-linux-gnu`
  - `sound-cabinet-v0.1.0-x86_64-unknown-linux-gnu.sha256`
  - `sound-cabinet-v0.1.0-aarch64-unknown-linux-gnu`
  - `sound-cabinet-v0.1.0-aarch64-unknown-linux-gnu.sha256`
  - `sound-cabinet-v0.1.0-aarch64-apple-darwin`
  - `sound-cabinet-v0.1.0-aarch64-apple-darwin.sha256`

#### Scenario: Checksum file format is compatible with sha256sum -c
- **WHEN** the workflow produces `sound-cabinet-v0.1.0-x86_64-unknown-linux-gnu.sha256`
- **THEN** the file contains exactly one line of the form `<64-hex-chars>  sound-cabinet-v0.1.0-x86_64-unknown-linux-gnu\n`
- **AND** running `cd <dir-containing-both-files> && sha256sum -c sound-cabinet-v0.1.0-x86_64-unknown-linux-gnu.sha256` succeeds with output containing `OK`

#### Scenario: Pre-release tag preserves the full tag in the asset name
- **WHEN** the tag `v0.2.0-rc1` is pushed
- **THEN** assets are named with `v0.2.0-rc1` verbatim (e.g. `sound-cabinet-v0.2.0-rc1-aarch64-apple-darwin`)
