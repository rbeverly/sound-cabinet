...
- [x] 1.1 In `src/dsl/expand.rs`, thread a set of section names that are
...
- [x] 1.2 In `expand_section` (`src/dsl/expand.rs:108`), before recursing into a
...
- [x] 1.3 Insert the section name into the active set on entry to
...
- [x] 1.4 Apply the same guard to BOTH `SectionEntry::Play`
  (`src/dsl/expand.rs:141-149`) and `SectionEntry::RepeatEvery`
  (`src/dsl/expand.rs:120-140`).

## 2. Tests
- [x] 2.1 Add a unit test `expand_rejects_self_referential_section` in
...
- [x] 2.2 Add a unit test `expand_rejects_mutually_recursive_sections` covering
  section `a` playing `b` and section `b` playing `a`, asserting `Err`.
- [x] 2.3 Add a unit test asserting that a section played twice from two
  ...
  set.
- [x] 2.4 Confirm existing tests `test_expand_section`,
